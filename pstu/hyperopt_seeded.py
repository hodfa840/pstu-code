"""Two-phase Optuna hyperparameter optimization for PSTU with configurable seed.

This is a version of hyperopt.py that accepts a seed parameter for reproducibility
...
def run_hyperopt_seeded(model_name, infected_path, clean_model_name,
                        n_trials=200, timeout=36000, group_size=2,
                        trim=False, output_dir=None, seed=42):
    """Run the full two-phase Optuna search with a specified seed.
    ...
    seed           : int  -- random seed for Optuna samplers
    """
    ...
    output_dir = Path(output_dir or f"results/pstu_seed{seed}")
    ...
    # Clean model PPL
    print(f"[1/5] Loading clean model... (seed={seed})")
    ...
    # Saliency (shared across seeds - no need to recompute)
    print("\n[2/5] Computing saliency...")
    sal_path = output_dir / f"{model_name}_saliency.json"
    # Also check parent directory for shared saliency
    shared_sal_path = Path(output_dir).parent / f"{model_name}_saliency.json"
    if sal_path.exists():
        with open(sal_path) as f:
            saliency = json.load(f)
        print(f"  Loaded cached saliency ({len(saliency)} params)")
    elif shared_sal_path.exists():
        with open(shared_sal_path) as f:
            saliency = json.load(f)
        print(f"  Loaded shared saliency ({len(saliency)} params)")
    else:
    ...
    # Optuna Phase 1: Pareto (fresh study with seeded sampler)
    half = n_trials // 2
    db_path = output_dir / f"{model_name}_seed{seed}.db"
    storage = f"sqlite:///{db_path}"

    print(f"\n[4/5] Phase 1: Pareto ({half} trials, seed={seed})...")
    pareto_study = optuna.create_study(
        study_name=f"{model_name}_pareto_seed{seed}", storage=storage,
    ...
    # Phase 2: Refinement
    print(f"\n[5/5] Phase 2: Refinement ({half} trials, seed={seed})...")
    refined_study = optuna.create_study(
        study_name=f"{model_name}_refined_seed{seed}", storage=storage,
        direction="minimize",
    ...
    # Save final results
    final = {
        "model": model_name, "seed": seed, "clean_ppl": clean_ppl,
    ...
    with open(output_dir / f"{model_name}_seed{seed}_final.json", "w") as f:
